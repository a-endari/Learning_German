...
def get_audio_url(german_word):
    ...
    if response.status_code == 200:
        ...
        div = soup.find(
            "h1", class_="mdc-typography--headline4 ltr d-inline position-relative"
        )
...
def download_audio(url, filename, convert=False):
    ...
    if response.status_code == 200:
        ...
        # Write the contents of the response (in this case a .wav audio file) into file_path_ogg
        with open(file_path_wav, "wb") as f:
            f.write(response.content)

        # Convert the file from .wav to .mp3 using ffmpeg if convert=True
        if convert:
            # Create the target file path for the .mp3 file.
            file_path_mp3 = os.path.join(folder_path, filename + ".mp3")
            convert_wav_to_mp3(file_path_wav, file_path_mp3)
            os.remove(file_path_wav)  # Remove the original .ogg file
